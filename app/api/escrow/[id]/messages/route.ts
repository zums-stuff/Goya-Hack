// app/api/escrow/[id]/messages/route.ts
// GET: lista mensajes del chat del escrow. Solo buyer + seller.
...
export async function GET(
  ...
  try {
    ...
    const acc = await resolveChatAccess({ scope: 'escrow', scopeId: id, userId: user.id });
    assertChatAccess(acc);

    const messages = await prisma.message.findMany({
      where: { scope: 'escrow', scopeId: id },
      orderBy: { createdAt: 'asc' },
      take: 200,
      include: { sender: { select: { id: true, displayName: true } } },
    });

    // Marca como leídos los mensajes que no son míos y aún sin readAt.
